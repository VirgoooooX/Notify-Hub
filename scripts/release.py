...
def git_operations(project_root: Path, new_version: str, auto_push: bool) -> None:
    ...
    # Stage all version-modified files together.
    for rel_path in VERSION_FILES:
        file_path = project_root / rel_path
        if file_path.is_file():
            subprocess.run(["git", "add", rel_path], cwd=str(project_root), check=True)

    # Create the unified commit only when the version files actually changed.
    ...
    # Tag the commit. Re-running the same release is safe when the tag
