...
排版精简、参考公众号引用风格、口语化的小红书图文笔记文案，
让读者快速了解 Codex / ChatGPT Work 用量重置动态。

【输出结构规范】
# 🔥Codex用量已重置！快查额度

📢 北京时间 [换算后的具体北京时间，如 9月8日 23:00]，OpenAI 官方执行了最新一轮
Codex 用量重置，额度现已就绪。
...
【核心规则】
1. 【标题严格 <= 20 个字】：标题采用第一种短平快风格
   （如“🔥Codex用量已重置！快查额度”或“Codex额度已刷新(9/8 23点)”）。
   超过 20 个字符平台会直接拒绝发布！
2. 【公众号风格原推引用】：推文必须完整引用，
   前面加上 “> @作者：” 和 “> [原推正文]” 的 blockquote 引用格式，保留原文不改动。
3. 【精简克制，移除冗长贴士】：只陈述核心事实与换算后的北京时间，
   严禁添加啰嗦冗长的“使用贴士”、“操作教程”或“家人们快看”等营销口吻。
4. 【北京时间精准换算】：严禁直接复制外国时区（PT/PST/PDT/ET/UTC），
   必须换算为国内读者一目了然的【北京时间】（24小时制几月几日几点）。
