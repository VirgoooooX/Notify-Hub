...
import{computed}from'vue';import{useRoute,useRouter}from'vue-router';import{Activity,BellRing,Users,KeyRound,Blocks,AlarmClock,Settings,Menu,LogOut,X,Bot,Cpu}from'lucide-vue-next';import{useAuthStore}from'@/stores/auth';import{useUiStore}from'@/stores/ui'
const route=useRoute(),router=useRouter(),auth=useAuthStore(),ui=useUiStore();const title=computed(()=>String(route.meta.title??'控制台'));const nav=[['/','概览',Activity],['/notifications','通知',BellRing],['/reminders','提醒',AlarmClock],['/plugins','插件',Blocks],['/ai/providers','AI 服务',Bot],['/ai/profiles','AI Profiles',Cpu],['/people','接收人',Users],['/api-clients','API Clients',KeyRound],['/settings','设置',Settings]];async function logout(){await auth.logout();router.push('/login')}
</script>
<template>
  <div class="app-shell">
