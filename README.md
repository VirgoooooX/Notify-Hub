...
- 📰 **公众号文章发布**
  ...
  3. **Browser Publisher 统一发布网关模式**（`browser`）：Notify Hub 只提交完整文章任务，独立容器负责调用官方 API 创建草稿，再由 Playwright 打开草稿并执行最后的「发表」动作；公众号 AppID/Secret 只放在发布器，带登录态二维码告警与防重复群发防线。
