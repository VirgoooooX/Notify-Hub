...
- 🧭 **多应用 Profile 运行时**
  在同一套核心、数据库和 Worker 中隔离多套企业微信 Agent。通知、提醒、回调、会话、交互状态、成员和媒体引用均带有明确 Profile 边界，后台可管理 Profile、成员、能力和加密凭据。

- 📰 **公众号文章发布**
  插件以 `publish_to_mp` 声明发布意图并提供文章内容与封面。支持三种模式：
  1. **官方 API 模式**（`draft`/`publish`）：适用于认证订阅号与服务号，核心负责上传永久素材、建草稿与发布；
  2. **文章库模式**（`library`）：文章进入工作台，可人工预览、复制公众号富文本格式确认发布；
  3. **Browser Publisher 统一发布网关模式**（`browser`）：Notify Hub 提交完整文章任务；正文中引用的 HTTP 图片先由 Notify Hub 安全下载并上传到发布器，独立容器再调用官方 API 创建草稿，最后由 Playwright 打开草稿并执行「发表」动作；公众号 AppID/Secret 只放在发布器，带登录态二维码告警与防重复群发防线。
  是否发布仍由插件确定性判定与 AI 置信度阈值决定，AI 只做翻译和摘要。
...
插件均包含 Manifest、配置模型、固定测试数据和单元测试。X 插件的时间线由平台统一读取；部署可用 twscrape 作为主源，失败时单次降级到 RSSHub，后台只配置账号、规则、接收人、AI Profile 与调度。RSSHub Token、X Cookie 和 Provider 不进入插件页面。平台健康 Worker 会把数据源/账号故障、自动降级、恢复和内容长期静默通过核心投递链路通知企业微信。
